...
func _physics_process(delta):
	...
	if elements_handler.guardians_possessed>0:
		protector = get_tree().get_nodes_in_group("protectors")[0]
		
		if abs(position.x-protector.position.x) < abs(position.x-player.position.x) and abs(position.y-protector.position.y) < abs(position.y-player.position.y):
			looking = protector.position.angle_to_point(position)
			attack_loop2()
		else:
...
func attack_loop():
	if can_attack == true and attacking == false and abs(position.x-player.position.x)<45 and abs(position.y-player.position.y)<45 and damage==false:
		attacking = true
		target = looking
...
func attack_loop2():
	if can_attack == true and attacking == false and abs(position.x-protector.position.x)<30 and abs(position.y-protector.position.y)<30 and damage==false:
		attacking = true
		target = looking
		weapon.rotation_degrees = looking*180/PI
		weapon.visible = true
